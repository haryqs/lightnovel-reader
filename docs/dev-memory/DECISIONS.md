...
3. 发现更新后必须先展示目标版本与发布说明并由用户确认；安装包由 Tauri updater 下载和验签，安装完成后
   才调用 process relaunch。测试 smoke 永远选择“稍后”，防止自动化误安装。
...
## 2026-08-21：更新确认采用应用内安全详情弹窗

决策：

1. updater 不再使用系统 `window.confirm`；发现新版本后打开原生 `<dialog>`，展示当前/目标版本、可选发布日期、
   发布说明和签名/重启提示。
2. Release body 只通过 `textContent` 作为纯文本显示，最长 4000 字符；不解析 Markdown，不接受 HTML 注入。
3. “稍后”是默认焦点，关闭按钮与 Escape 都明确映射为稍后；只有显式点击“下载并安装”才进入下载流程。
4. smoke 独立验证弹窗 ARIA 标题/描述、两个操作、打开/关闭，并在未来检测到新版本时自动点击“稍后”。

理由：

- 系统确认框无法清晰容纳版本对比、较长发布说明和签名来源信息，也无法与应用主题保持一致。
- Release body 属于远端输入，即使来源是自己的 GitHub Release，也应坚持纯文本渲染，避免未来维护流程引入 HTML 风险。
- 安装会下载可执行文件并重启应用，安全默认值应是不执行，而不是把 Enter/Escape 隐式绑定到安装。
...
## 2026-08-22：用户数据备份先做可校验导出，恢复后置

决策：

1. `userData.exportBackup` 作为冻结候选协议上的新增消息；平台壳只选择目标目录，reading-core 持有备份格式与文件策略。
2. `reader.db`、`library.sqlite` 从各自持锁连接执行 `VACUUM INTO`，不复制活动数据库；书库资产与插件目录按文件复制。
3. 备份先写同级 `.partial`，逐文件记录并复核 SHA-256 后才改为正式目录；拒绝把目标放在 app data 内。
4. 缓存、SQLite WAL/SHM 与含同步令牌的 `sync.json` 明确排除。schema v1 暂不提供恢复命令。

理由：

- 直接复制活动 SQLite 文件可能漏掉 WAL 中已提交数据，不能称为可靠备份。
- 恢复会覆盖用户现有进度、标注、书库与插件状态，风险显著高于只创建新目录的导出；先稳定格式和校验链更安全。
- 目标路径和小型摘要足以跨桥接传递，数据库与 EPUB 字节无需进入 JSON/IPC。

后果：

- 备份可独立搬运并离线验完整性，但 `manifest.json` 与载荷目前未加密；用户应保存到受信目录。
- 插件私有 KV 随插件数据进入备份；同步配对凭据不进入备份，迁移后需要重新配对。
- 后续恢复必须先新增只读 schema/哈希/内容预览与版本兼容检查，再设计明确的冲突和回滚策略。

## 2026-08-23：备份校验是独立只读消息，恢复事务继续后置

决策：

1. 新增 additive `userData.inspectBackup`，桌面壳只负责选择一个既有目录；所有 schema、路径、文件集合、
   SHA-256、SQLite 完整性与内容计数规则归 `reading-core::backup`。
2. 校验要求清单载荷与实际普通文件精确相等，拒绝绝对/穿越/反斜杠/冒号路径、重复项、符号链接、
   `manifest.json` 自引用、缓存、同步凭据和 SQLite WAL/SHM；摘要通过后再以只读连接执行 `PRAGMA quick_check`。
3. 来源应用版本高于当前版本时允许完成只读校验，但必须返回 `newerThanCurrentApp` 与显式 warning；界面只展示
   书籍、进度、标注、插件、EPUB、载荷和版本摘要，没有“恢复”按钮。
4. “校验通过”只表示当前 schema 下的载荷与数据库可读，不表示兼容合并或授权覆盖当前 app data。

理由：

- 先把不受信备份目录变成可严格验证的小型摘要，才能为未来恢复建立稳定输入边界。
- 目录中额外文件、符号链接和路径穿越即使不在清单内，也可能在未来复制实现中扩大作用域，因此必须现在拒绝。
- 版本兼容、冲突处理和回滚是恢复事务问题，不应混入只读检查或用一个布尔值过早承诺。

后果：

- 冻结候选协议只增加可选能力，数据库和 EPUB 字节仍不进入 IPC；Web 壳明确不支持该桌面能力。
- 下一阶段必须先形成恢复事务设计：恢复前自动备份当前数据、离线替换/迁移、失败回滚、冲突预览和二次确认。
- 发下一版本前仍需人工点击 Windows 保存与校验目录选择器；自动化只覆盖命令和应用内详情弹窗。

## 2026-08-23：恢复采用整体替换事务，先交付只读计划

决策：

1. 新增 additive `userData.planRestore`。它必须重新执行完整备份校验，再持有当前两份 SQLite 连接的锁，
   只读比较当前与备份的书籍、进度、标注、插件和 EPUB 数量。
2. 首版恢复语义固定为整体替换，不做逐表/逐文件自动合并。计划必须返回回滚点预计大小、替换文件数，
   并固定声明 `requiresPreRestoreBackup=true`、`requiresRestart=true`。
3. 较新应用版本生成的备份允许校验和查看，但 `versionCompatible=false` 并给出阻断原因；不能进入未来执行事务。
4. 当前只显示计划，不增加恢复按钮或执行消息。备份来源位于 app data 内时直接拒绝，避免未来替换时同时删除来源。

理由：

- 当前 SQLite 连接在应用进程内持续活动，直接替换数据库文件会产生平台相关且不可可靠回滚的状态。
- 自动合并进度、标注、书目关系和插件私有状态需要各自冲突语义；未经设计的“智能合并”比整体替换更难验证。
- 用户必须在执行前准确看到当前值将变成什么、回滚需要多少数据量，以及为何需要重启。

后果：

- 下一阶段执行器应采用退出活动连接后的 staging/替换/复核事务，失败时使用外部预备份回滚。
- 磁盘空间检查、外部回滚目录选择、二次确认、启动事务标记和故障注入测试未完成前，不开放执行恢复。

## 2026-08-24：恢复准备先创建外部回滚点和可审计凭据，仍不安排执行

决策：

1. 新增 additive `userData.prepareRestore`；只有 `userData.planRestore` 版本兼容且无阻断原因时才能进入准备。
2. 平台壳只选择外部回滚父目录；reading-core 复用正式备份链从活动连接生成当前数据快照，并用完整备份检查再次复核。
3. 回滚点完成后必须重新完整校验恢复来源，再在外部目录原子写 JSON 准备凭据，钉住来源和回滚
   `manifest.json` 的 SHA-256；凭据固定 `restoreExecuted=false`、`requiresRestart=true`。
4. 本阶段不写 app data、不关闭 SQLite、不创建启动执行标记，也不把按钮命名为“恢复”；UI 明示“创建外部回滚点（不恢复）”。

理由：

- 真正替换前必须先证明存在独立、完整、可定位的当前数据退路；只估算回滚大小不足以支持失败恢复。
- 凭据和回滚点留在 app data 外，可避免未来整体替换把自己的恢复证据一并删除。
- 创建回滚点可能耗时，结束时重新校验来源并记录 manifest 摘要，可为下一阶段防止来源 TOCTOU 漂移提供稳定输入。

后果：

- 用户可提前完成恢复准备，但当前数据不会发生替换，重启应用也不会自动恢复。
- 下一阶段必须重新校验凭据、来源和回滚摘要，补磁盘空间检查、二次确认、启动期 staging/替换/复核与故障注入自动回滚。

## 2026-08-24：恢复预检是可重复的只读阻断门，不是执行授权

决策：

1. 新增 additive `userData.preflightRestore`；core 读取外部准备凭据后重新校验普通文件/路径/状态，完整检查来源与
   回滚备份，并核对凭据钉住的两份 manifest SHA-256。
2. app data 所在卷空间由 native-only `fs2 0.4.3` 查询普通用户实际可用字节；恢复 staging 需求固定为来源载荷
   加 `max(来源载荷 10%, 64 MiB)` 安全余量，空间不足返回显式阻断结果。
3. 预检结果固定 `requiresFreshRollbackAtExecution=true`、`restoreAuthorized=false`、`restoreExecuted=false`；
   准备后用户可能继续阅读，真正执行时必须在持锁状态刷新回滚点，历史预检不得直接转成覆盖授权。

理由：

- 只校验准备时记录的摘要会忽略之后的磁盘漂移；执行前需要一个可重复、无写入的复核门。
- Rust 标准库尚无跨平台可用空间 API；`fs2` 只进入 native feature，依赖面小，并统一 Windows/Unix 语义。
- 小书库如果只按百分比留余量会过于激进，固定 64 MiB 下限可覆盖 manifest、文件系统分配粒度和事务元数据。

后果：

- 用户可在不重建回滚点的情况下重复复核凭据和空间，但预检按钮仍明确标注“不恢复”。
- 下一阶段还需二次确认、持锁刷新回滚点、启动期 staging/替换/复核、失败回滚和故障注入；当前没有执行消息。

## 2026-08-24：启动期恢复先落隔离事务内核，不直接开放执行入口

决策：

1. native `reading-core` 新增低层恢复事务内核，但本阶段不增加 Tauri command、桥接消息、启动消费标记或 UI 按钮。
2. 事务目录固定为 app data 的同级隐藏目录；不可变 `request.json` 钉住应用目录、来源/回滚路径及两份 manifest
   SHA-256，阶段以仅创建一次的 `staged/displacing/current-displaced/activating/source-activated` 标记推进。
3. 当前 `reader.db`、WAL/SHM、`library/` 与 `plugins/` 先以同卷 rename 移入 `displaced/`，再从完整 staging 激活
   来源的 `reader.db/library/plugins`；`sync.json`、缓存及未知顶层状态保持原位，不进入整体替换。
4. 激活后必须精确匹配来源文件集合、大小、SHA-256，并对两份 SQLite 执行 `PRAGMA quick_check`；失败优先使用
   `displaced/` 中的原始数据幂等回滚。外部回滚备份继续作为灾难退路，不作为进程中断后本地回滚的在线依赖。
5. 遗留事务必须先于外部来源复核处理：来源未完整激活时先恢复原数据；来源已激活时使用 staging 内保留的
   manifest 独立复核并收尾。初始化阶段的不完整事务可安全丢弃重建，已开始移动但请求无效时则阻断人工处理。

理由：

- 退出连接后的多个文件/目录替换无法由单次文件系统操作原子覆盖，必须把每一阶段和原始集合持久化，才能在
  任意进程中断点判定“继续完成”还是“回到原状态”。
- 外置 USB、网络盘或用户选择目录可能在重启前离线；已开始改变 app data 后仍依赖它们才能回滚会放大数据风险。
- 先用隔离临时目录和故障注入证明事务语义，可把高风险文件替换与后续授权、退出/重启编排分开审查。

后果：

- 6 个 native 测试覆盖成功、初始化残留、staging 后中断、部分移出/激活、复核失败、回滚再次中断、重复启动，
  以及外部来源/回滚目录消失后的本地自救；这些测试只操作系统临时目录。
- 下一阶段仍必须在两份活动数据库持锁时重新预检并刷新回滚点，经明确二次确认后原子写一次性启动请求，再让
  平台壳退出连接并重启消费；真实隔离 app data smoke 全绿前不得开放执行按钮。

## 2026-08-24：恢复授权只生成一次性启动请求，刷新回滚点与发布必须同处持锁调用

决策：

1. core 新增 `schedule_user_data_restore_for_startup`，但不增加 Tauri command、ReaderBridge 消息或 UI。
2. 调用方必须同时提供精确短语“恢复并替换当前数据”、`confirmsFullReplacement=true` 与
   `confirmsRestart=true`；任一缺失都在读取凭据或创建新文件前拒绝。
3. 调度先重跑历史准备凭据预检，再复用活动 `reader.db/library.sqlite` 连接在原回滚父目录创建全新的当前数据
   回滚点和凭据；刷新后再次完整预检，并要求来源 manifest 摘要与确认前一致，防止确认对象发生漂移。
4. 一次性请求固定写入 app data 的 `.restore-startup-request-v1.json`，记录来源、新鲜回滚点、凭据、自身/app data
   路径和两份摘要，并固定 `requiresRestart=true/restoreAuthorized=true/restoreExecuted=false`。
5. 请求先完整写入并同步 `.partial`，再以同 inode 硬链接发布最终名称，确保最终路径只会从“不存在”变为完整文件；
   已有 final/partial 均阻断重复调度。发布前失败只删除本次新建的回滚目录与凭据，历史退路保持不变。

理由：

- 历史回滚点可能早于用户最后一次阅读或标注修改；调度与新快照若不共用仍被调用方持有的数据库锁，就无法证明
  退路覆盖确认瞬间的当前状态。
- 二次确认之后来源仍可能被外部修改；刷新回滚点耗时结束后必须再校验并与确认前摘要对照，不能静默改换目标。
- 直接覆盖固定请求或先发布半写文件，都会让重复点击、并发调用或进程中断产生不可判定状态；create-new 语义和
  完整文件发布把失败收敛为“没有请求”或“恰好一个完整请求”。

后果：

- 调度成功只意味着已准备可靠输入并请求下次启动执行；当前进程不会关闭数据库，也不会替换任何受管数据。
- 下一阶段必须在 SQLite 初始化前校验/消费该请求，写完成或失败结果日志并证明一次性语义；在隔离进程 smoke
  完成前，不得把 core 原语暴露给正式 UI。
