...
1. **准备 v0.7.1 维护版本**：优先收集 v0.7.0 的安装、启动、阅读和插件来源反馈，修复首发问题；保持现有
   updater 密钥与公钥不变。
2. **完成真实跨版本 updater 验证**：发布 v0.7.1 候选时，先从公开安装的 v0.7.0 点击应用内更新入口，验证
   检查、下载、签名拒绝/接受、安装、重启与用户数据保留；当前只验证了“无新版本”的检查分支。
