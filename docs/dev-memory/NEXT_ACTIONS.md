...
1. **合并新 updater 公钥**：本地完整检查与远端 CI 通过后，把密钥轮换提交合并到 `main`；私钥和密码不得提交。
2. **准备 v0.7.0 草稿**：上传已验收五资产到新的 Draft Release，并以大小/SHA-256 与本机候选逐项核对。
3. **人工审阅再公开**：确认说明、许可证、目标提交和资产无误后，需维护者明确同意才公开 Release。
4. **发布后在线复验**：公开后验证 `latest.json`、插件仓库下载和真实更新重启；完成前不宣称在线更新闭环通过。
