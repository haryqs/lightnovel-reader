...
1. **实现启动期恢复事务**：先增加磁盘空间检查、准备凭据与两份清单重新校验和明确二次确认，再由应用退出活动连接后的一次性启动事务执行
   staging/替换/复核/失败回滚。故障注入测试齐备前不得开放“执行恢复”。
2. **补人工选择器验收**：发下一版前人工点击保存、校验和回滚点三个 Windows 原生目录选择器，确认取消、重选和异常路径文案。
