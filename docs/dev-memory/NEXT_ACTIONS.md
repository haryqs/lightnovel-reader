...
## 📌 交接留言（2026-08-22，v0.7.3 updater 收口候选）

当前事实：

- 公开 Latest 仍为 v0.7.2；npm、Tauri 与三个第一方 Cargo 包已统一升至 v0.7.3 源码候选，尚未生成、上传或公开 v0.7.3 资产。
...
1. **合并 v0.7.3 修复**：完成项目门、前端构建、Rust workspace/QuickJS、真实窗口 smoke 与 PR/main CI。
2. **生成 v0.7.3 候选**：从合并后的固定 main 提交交互输入 updater 密码，构建并公钥验收严格五资产；先创建 Draft，不自动公开。
3. **发布与监测**：仅在维护者再次明确授权后公开 v0.7.3，再从 v0.7.2 复验 quiet 更新和数据保留；继续收集 v0.7.2 安装/更新反馈。
