...
- PR #65 已把安全导出合并到 `main`。当前增量新增只读 `userData.inspectBackup`：精确载荷集合、路径/链接、
  大小/SHA-256、SQLite `quick_check` 与来源版本检查，并在桌面弹窗预览内容计数；仍无恢复命令。
- PR #66 已把只读校验与预览合并到 `main@503b698`，PR/main 两套 Windows CI 全绿。当前增量新增
  `userData.planRestore`，只读比较当前/备份内容、估算回滚点并暴露版本阻断与重启要求；仍无执行恢复消息。

下一步优先级：

1. **完成恢复计划切片**：全套测试、真实 Tauri/P0 smoke、PR/main CI 后合并；发下一版前人工点击保存与校验目录选择器。
2. **设计恢复执行器**：恢复前在用户指定外部目录自动导出当前数据；应用退出活动连接后由一次性启动事务执行
   staging/替换/复核/失败回滚。二次确认、磁盘空间检查和故障注入测试齐备前不得开放执行按钮。
3. **监测 v0.7.3**：收集安装、启动、阅读、更新入口和插件来源反馈；只通过新版本修复，不替换既有 Release 资产。
