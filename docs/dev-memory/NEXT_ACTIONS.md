...
1. **合并书架整理体验**：提交筛选、排序、结果计数和 smoke 增强，等待 PR/main CI 全绿后合并。
2. **监测 v0.7.3**：收集安装、启动、阅读、更新入口和插件来源反馈；只通过新版本修复，不替换既有 Release 资产。
3. **下一体验切片**：优先评估用户数据备份/恢复入口或安全的书架移除语义；先完成数据边界设计，再决定是否改协议。
