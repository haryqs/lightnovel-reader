...
## 📌 交接留言（2026-08-21，v0.7.2 Draft Release 已就绪）
...
- v0.7.2 已从固定提交 `9c7d36f` 生成签名五资产并通过统一验收；本机安装、安装版真实窗口更新检查、卸载和
  两份数据库哈希保留均通过。Draft Release 固定到该完整提交，远端五资产摘要逐项匹配，公开 tag 不存在。

下一步优先级：

1. **确认并公开 v0.7.2 后完成 updater 闭环**：Draft 仍不得自行公开；获得维护者单独明确同意后发布，再从
   正式安装的 v0.7.1 点击应用内入口，验证版本/日期/发布说明、确认、真实进度、签名接受、安装、重启和
   用户数据保留。签名拒绝路径使用篡改副本离线验证，不得替换公开资产。
2. **监测 v0.7.1**：收集安装、启动、阅读、更新入口和插件来源反馈，优先修复公开版本问题。
3. **继续产品主线**：在 v0.7.1 稳定性可控后选择 v0.7 后续体验收口或 v0.8 Android 的最小切片；GitHub
