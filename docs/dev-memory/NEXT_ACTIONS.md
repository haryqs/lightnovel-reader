...
- PR #65/#66/#67 已依次把安全导出、只读校验预览和恢复事务计划合并到 `main@254630d`，PR/main Windows CI 全绿。
- 当前增量新增 `userData.prepareRestore`：先阻断不兼容来源，在用户选择的外部目录创建当前数据一致性回滚备份，
...
1. **设计一次性启动恢复事务和明确二次确认**：执行入口必须在持锁状态刷新当前数据回滚点，再由应用退出活动连接后执行
   staging/替换/复核/失败回滚；历史预检不能直接变成执行授权。
2. **先补故障注入执行器测试**：覆盖 staging 中断、单文件替换失败、复核失败、回滚失败与重复启动幂等；全绿前不得开放“执行恢复”。
