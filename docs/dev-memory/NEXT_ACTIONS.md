...
## 📌 交接留言（2026-08-24，一次性启动恢复调度内核已完成）

当前事实：

- 公开 Latest、npm、Tauri 与三个第一方 Cargo 包均为 v0.7.3。公开 tag 精确指向构建提交 `60c0868`，五资产和 Latest 别名均已从公开 URL 重新下载验收。
- 正式安装的 v0.7.1 已通过应用内入口更新至公开 v0.7.2，覆盖详情、下载、验签、安装、重启、新 UI 和同版本检查；两份数据库哈希不变。
- v0.7.3 将 PWA Service Worker 限制到 Web；Windows 在 WebView2 创建前一次性删除旧 Cache/Code Cache/Service Worker，前端再注销残留注册并清 CacheStorage。updater 固定为 `quiet`，避免多语言 NSIS 的语言选择框阻塞自动安装。
- `smoke:updater` 始终只检查并安全取消；真实安装只能使用带显式确认参数的 `scripts/tauri-updater-install-smoke.mjs`，且必须先备份用户数据。真实安装脚本现自动兼容旧 `window.confirm` 与当前详情 `<dialog>`。
- 公开 v0.7.2 二进制仍是旧 `passive` 安装配置，部分用户更新时可能看到语言选择框；不得替换其既有资产，修复通过 v0.7.3 正常发布交付。
- 已从正式 v0.7.2 经应用内详情弹窗更新至公开 v0.7.3，覆盖发布说明、安全取消、再次确认、下载、验签、quiet 安装、自动重启、新 UI 与同版本 Latest 检查；没有语言选择框，两份数据库哈希不变。
- 发布后首个体验切片已实现书架筛选、排序和结果计数；隔离两本 EPUB 的 P0 smoke 已覆盖最近阅读、本地未读、远程空筛选与书名排序，不改冻结协议。
- 用户数据备份 v1 已实现：两份 SQLite 使用一致性快照，书库资产与插件数据进入带 SHA-256 的 schema v1 清单；缓存、WAL/SHM 和同步令牌排除。真实 P0 已从 Tauri 命令生成并检查备份。
- PR #65—#70 已依次把安全导出、只读校验预览、恢复计划、外部回滚准备、只读预检和启动期事务内核合并到
  `main@2c01556`，PR/main Windows CI 全绿。
- `userData.prepareRestore` 会先阻断不兼容来源，在用户选择的外部目录创建当前数据一致性回滚备份，
  完整复核后重新校验来源，再原子写入钉住两份 manifest SHA-256 的 JSON 准备凭据。固定
  `restoreExecuted=false`；没有关闭连接、替换数据或启动期执行标记。
- `userData.preflightRestore` 会重新复核外部普通 JSON 凭据、来源/回滚全部载荷与两份 manifest 摘要，并读取
  app data 所在卷可用空间；staging 需求为来源载荷加 `max(10%, 64 MiB)` 安全余量。固定
  `requiresFreshRollbackAtExecution=true`、`restoreAuthorized=false`、`restoreExecuted=false`。
- `reading-core::restore_transaction` 已新增尚未接平台壳的启动期事务内核：同卷固定事务目录依次执行
  staging、移出当前受管数据、激活来源、精确 SHA-256/SQLite 复核和失败回滚；不可变请求及阶段标记支持重启续跑。
- 6 个隔离故障测试覆盖初始化残留、staging 后中断、部分移出/激活、复核失败、回滚再次中断、来源/回滚外置目录
  丢失后的本地自救和来源已激活后的幂等收尾。`sync.json`、缓存及未知顶层状态不参与替换。
- 当前仍没有恢复执行消息、按钮或启动消费；低层事务内核本身不负责刷新回滚点。
- 当前增量新增尚未接平台壳的 `schedule_user_data_restore_for_startup`：调用方必须提供精确短语
  “恢复并替换当前数据”并确认整体替换/重启；core 先复核历史凭据，再从仍持锁的两份 SQLite 刷新当前数据
  外部回滚点、重新预检并以同 inode 硬链接原子发布 app data 内固定的一次性 JSON 请求。
- 调度成功不会关闭连接或替换数据；失败只清理本次新建的回滚目录与凭据。重复请求、来源漂移、不完整确认均在
  再次刷新前阻断。恢复事务模块共 11 个隔离测试，真实 app data 未读取或修改。

下一步优先级：

1. **实现启动请求校验、消费与结果日志**：在初始化两份 SQLite 前严格复核固定请求、自身路径、来源/回滚/凭据
   和摘要，再调用事务内核；完成/失败必须写可审计结果并确保同一请求不会重复执行。
2. **接入真实隔离进程 smoke**：平台壳只在测试开关与独立 app data 下先接消费链，完成真实退出、重启、成功恢复、
   失败回滚和重复启动验收；全绿后再设计 UI 二次确认与退出编排，不得先开放正式按钮。
3. **补人工选择器验收并监测 v0.7.3**：发下一版前人工点击三个 Windows 原生目录选择器；继续收集公开版本反馈，
   只通过新版本修复，不替换既有 Release 资产。

## 📌 交接留言（2026-08-22，v0.7.2 已正式公开）

当前事实：

- npm、Tauri 与三个第一方 Cargo 包版本已统一为 `0.7.2`，运行时 User-Agent 从 Cargo 包版本生成；
...
- 新 updater 公钥已通过 PR #48 合并到 `main`（`cb75a53`）。`v0.7.0` 已以该合并提交为目标正式公开为
  Latest Release；公开五资产重新下载后通过统一验收，远端集合、大小与 SHA-256 全部匹配本机候选。
- `/releases/latest/download/latest.json` 与版本化清单完全一致；公开下载的 NSIS 已再次通过静默安装、实际启动、
  静默卸载和用户数据保留验证。
- 发布密钥已在两个仓库外本地卷完成独立备份并逐文件核对 SHA-256，备份目录只允许当前 Windows 账户访问；
  updater 密码未写盘，必须由维护者另存密码管理器。
- 桌面端已增加手动“检查更新”：通过 `ReaderBridge` 调用 Tauri updater/process，确认后下载、验签、安装并重启；
  Web/PWA 隐藏入口。`smoke:updater` 已在真实窗口访问公开 v0.7.0 清单并正确返回“当前已是最新版本”。
- 该 UI 在 v0.7.0 正式发布后才合并，因此公开 v0.7.0 二进制没有更新按钮；v0.7.0 只可用于底层 updater
  链路验证。v0.7.1 已成为首个带界面更新入口的公开版本，完整界面闭环需由 v0.7.1 更新到后续版本验证。
- v0.7.1 已从 `main` 合并提交 `1929999` 生成签名五资产候选并通过统一验收；经维护者明确同意后已正式公开
  为 Latest Release。公开五资产、Latest 别名、安装版真实窗口、更新检查、卸载及两份数据库哈希保留均复验通过。
- v0.7.2 已从固定提交 `9c7d36f` 生成签名五资产并通过统一验收；本机安装、安装版真实窗口更新检查、卸载和
  两份数据库哈希保留均通过。经维护者明确同意，现已正式公开为 Latest Release；tag 固定到该完整提交，
  远端五资产摘要逐项匹配。

下一步优先级：

1. **完成 v0.7.1→v0.7.2 updater 闭环**：从正式安装的 v0.7.1 点击应用内入口，验证版本/日期/发布说明、
   确认、真实进度、签名接受、安装、重启和
   用户数据保留。签名拒绝路径使用篡改副本离线验证，不得替换公开资产。
2. **监测 v0.7.2**：收集安装、启动、阅读、更新入口和插件来源反馈，优先修复公开版本问题。
3. **继续产品主线**：在 v0.7.2 稳定性可控后选择 v0.7 后续体验收口或 v0.8 Android 的最小切片；GitHub
   Support #4676102 可在官方回复确认账户正常后关闭。
