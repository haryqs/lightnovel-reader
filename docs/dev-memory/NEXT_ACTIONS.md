...
## 📌 交接留言（2026-08-21，v0.7.1 发布准备中）

当前事实：

- npm、Tauri 与三个第一方 Cargo 包版本已统一为 `0.7.1`，运行时 User-Agent 从 Cargo 包版本生成；
...
- 该 UI 在 v0.7.0 正式发布后才合并，因此公开 v0.7.0 二进制没有更新按钮；v0.7.0 只可用于底层 updater
  链路验证。v0.7.1 将是首个带界面更新入口的公开版本，完整界面闭环需由 v0.7.1 更新到后续版本验证。
- v0.7.1 已从 `main` 合并提交 `1929999` 生成签名五资产候选并通过统一验收；静默安装、安装版真实窗口、
  更新入口、静默卸载和两份数据库前后哈希保留均通过。GitHub Draft Release 已创建并核对，仍不得擅自公开。

下一步优先级：

1. **等待 v0.7.1 公开确认**：草稿目标已固定为 `1929999`，远端五资产名称、大小和 SHA-256 已逐项匹配；
   必须等待维护者明确同意才公开。公开后重新下载五资产并复验安装与数据保留。
2. **分层验证 updater**：v0.7.1 公开后，可从公开 v0.7.0 通过底层 updater 接口验证下载、签名接受、安装和
   数据保留；完整用户界面检查、确认、安装与重启闭环必须从 v0.7.1 更新到后续版本验证。
3. **继续产品主线**：在 v0.7.0 稳定性可控后选择 v0.7 后续体验收口或 v0.8 Android 的最小切片；GitHub
