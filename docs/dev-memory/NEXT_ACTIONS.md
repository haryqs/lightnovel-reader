...
## 📌 交接留言（2026-08-22，v0.7.3 Draft 已就绪）

当前事实：

- 公开 Latest 仍为 v0.7.2；npm、Tauri 与三个第一方 Cargo 包为 v0.7.3。修复已通过 PR #61 合并到 `main@60c0868`，PR/main Windows CI 均全绿。
...
- `smoke:updater` 始终只检查并安全取消；真实安装只能使用带显式确认参数的 `scripts/tauri-updater-install-smoke.mjs`，且必须先备份用户数据。
- 公开 v0.7.2 二进制仍是旧 `passive` 安装配置，部分用户更新时可能看到语言选择框；不得替换其既有资产，修复通过 v0.7.3 正常发布交付。
- 已从固定构建提交生成并验收 v0.7.3 严格五资产；静默安装、安装版 smoke、首次缓存迁移、卸载数据保留和重装通过。GitHub Release 为 Draft、非预发布，目标提交与五资产摘要均匹配，公开 tag 不存在。

下一步优先级：

1. **等待明确公开授权**：Draft 已就绪，但不得因为构建/验收通过而自动公开；发布前再次核对标题、说明、目标提交与五资产。
2. **发布后跨版本复验**：从正式安装的 v0.7.2 经应用内入口更新到 v0.7.3，验证 quiet 安装无语言框、重启、新 UI 和数据保留。
3. **监测与后续主线**：继续收集 v0.7.2/v0.7.3 反馈，稳定后再选择 v0.7 体验收口或 v0.8 Android 最小切片。
