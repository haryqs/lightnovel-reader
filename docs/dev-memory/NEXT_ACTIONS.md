...
## 📌 交接留言（2026-08-23，备份只读校验与内容预览开发中）
...
1. **完成只读校验切片**：全套测试、真实 Tauri/P0 smoke、PR/main CI 后合并；发下一版前人工点击保存与校验目录选择器。
2. **设计恢复事务**：先定冲突预览、当前数据自动回滚备份、版本迁移和二次确认；方案评审前不得实现覆盖 app data。
