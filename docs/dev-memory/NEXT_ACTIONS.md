...
- PR #65—#69 已依次把安全导出、只读校验预览、恢复计划、外部回滚准备和只读预检合并到
  `main@8f610fc`，PR/main Windows CI 全绿。
...
- 当前仍没有恢复执行消息、按钮或一次性启动调度标记；低层内核也不负责刷新回滚点。真实用户数据不会因本增量被触碰。

下一步优先级：

1. **设计授权与调度外壳**：明确二次确认后，执行入口必须在持有两份数据库锁时重新跑预检并刷新当前数据回滚点，
   原子写一次性启动请求，再退出活动连接；历史预检不能直接变成执行授权。
2. **接入启动消费与真实隔离 smoke**：平台启动时只消费一次性请求并调用已验证内核，记录完成/失败结果；先在独立
   app data 目录完成真实退出、重启、成功恢复、失败回滚和重复启动验收，全绿前仍不得开放正式“执行恢复”按钮。
