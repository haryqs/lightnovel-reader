...
首个带应用内更新入口的公开版本已推进到 `v0.7.1`：npm、Tauri 与三个第一方 Cargo 包同步版本，并由自动检查防止漂移；
...
updater 后端、没有
更新按钮；v0.7.1 是首个界面可用版本。后续开发已补入应用内更新详情弹窗、下载字节/百分比、验签安装阶段
提示、重复安装合并与失败后重试状态；弹窗显示当前/目标版本、发布日期和纯文本发布说明，默认操作为“稍后”。
完整用户界面更新闭环仍需从公开 v0.7.1 更新到后续版本验证。
