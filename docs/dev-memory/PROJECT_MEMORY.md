...
签名五资产候选与 Draft Release 已就绪但尚未公开，并由自动检查防止版本漂移；
...
当前 v0.7.1 Latest 清单并走通无更新分支。该界面代码在 v0.7.0 发布后合并，公开 v0.7.0 二进制只有
...
- 2026-08-21：v0.7.2 已从 `main` 固定提交 `9c7d36f` 完成正式签名构建和五资产统一验收；安装版真实窗口
  ...
  不存在；公开 Latest 与 README 下载入口仍保持 v0.7.1，需维护者再次明确同意后才能公开。
