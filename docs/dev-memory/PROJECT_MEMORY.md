...
首个带应用内更新入口的公开版本是 `v0.7.1`；公开 Latest 是 `v0.7.2`，当前 npm、Tauri 与三个第一方 Cargo 包
已同步进入 `v0.7.3`，签名五资产 Draft 已就绪，并由自动检查防止版本漂移；
...
`quiet`，并以发布门与专用真实安装 smoke 防止回归。公开 v0.7.2 二进制仍包含旧 `passive` 配置，永久修复需发布 v0.7.3。
