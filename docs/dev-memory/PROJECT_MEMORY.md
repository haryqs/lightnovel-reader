...
首个带应用内更新入口的公开版本是 `v0.7.1`；公开 Latest 与 npm、Tauri、三个第一方 Cargo 包均为
`v0.7.3`，签名五资产已正式公开并由自动检查防止版本漂移；
...
候选逐项一致，公开安装器的安装、启动、卸载与数据保留复验通过。v0.7.1 保持同一 updater 信任根并已正式
公开，公开五资产与 `releases/latest` 清单也已复验。
桌面端现已通过 `ReaderBridge` 增加手动检查更新入口：Tauri 壳调用官方 updater/process 插件，发现更新后由
用户确认，再下载、验签、安装并重启；Web 壳隐藏入口。真实窗口 `smoke:updater` 已先后检查公开 v0.7.0 与
当时公开的 v0.7.1 Latest 清单并走通无更新分支。该界面代码在 v0.7.0 发布后合并，公开 v0.7.0 二进制只有
updater 后端、没有更新按钮；v0.7.1 是首个界面可用版本。v0.7.2 源码候选已补入应用内更新详情弹窗、
下载字节/百分比、验签安装阶段
提示、重复安装合并与失败后重试状态；弹窗显示当前/目标版本、发布日期和纯文本发布说明，默认操作为“稍后”。
真实 v0.7.1→v0.7.2 用户界面更新闭环已通过。测试同时暴露桌面 WebView2 可被旧 PWA Service Worker/HTTP
缓存污染，以及多语言 NSIS 在 updater `passive` 模式下弹出语言框并阻塞无人值守安装；v0.7.3 源码已将
Service Worker 限定到 Web/PWA、Windows 在 WebView2 创建前一次性清除旧缓存且前端注销残留注册，同时固定 Windows updater 为
`quiet`，并以发布门与专用真实安装 smoke 防止回归。v0.7.3 已正式发布，且从公开 v0.7.2 通过应用内入口
完成 quiet 安装、自动重启与数据保留复验；v0.7.2 的既有公开资产保持不变。
发布后体验收口新增纯前端书架整理：按可阅读/远程收藏/本地未读筛选，按最近阅读/最近加入/书名/作者排序，
并显示当前结果数；复用既有 `LibraryBook` 字段，不扩展冻结协议。
桌面数据安全切片新增 `userData.exportBackup`：core 从持锁 SQLite 连接生成两份一致性快照，复制书库资产与
插件数据并写逐文件 SHA-256 清单；缓存、WAL/SHM 和含同步令牌的 `sync.json` 明确排除。随后新增
`userData.inspectBackup`：只读拒绝不安全路径、符号链接、载荷集合/摘要漂移并检查 SQLite 完整性，成功后
预览版本及书籍/进度/标注/插件/EPUB 数量。`userData.planRestore` 再只读比较当前与备份内容，估算外部
回滚点大小并声明整体替换、预备份、重启和版本阻断要求。`userData.prepareRestore` 已能在用户选择的外部目录
创建并完整复核当前数据回滚点，随后原子写入钉住来源/回滚 manifest SHA-256 的 JSON 准备凭据；固定记录
`restoreExecuted=false`。`userData.preflightRestore` 会重新完整复核凭据、来源与回滚点，并读取 app data 所在卷
可用空间，要求来源 staging 载荷加 `max(10%, 64 MiB)` 安全余量；它固定不授权、不执行，并要求执行前刷新
可能已陈旧的回滚点。native `reading-core::restore_transaction` 已实现尚未接平台壳的启动期低层事务：固定同卷
事务目录保存不可变请求与阶段标记，staging 后以 rename 移出当前受管数据、激活来源，精确复核文件集合、
SHA-256 和两份 SQLite，失败则幂等回滚；遗留事务会先用同卷 displaced 数据自救，不依赖外部备份仍在线。
core-only 调度原语进一步要求精确确认短语及整体替换/重启双确认，先复核历史凭据，再在两份活动 SQLite 仍
持锁时刷新外部回滚点、再次预检并原子发布 app data 内固定的一次性启动 JSON；重复请求、来源漂移或发布失败
会阻断并清理本次刷新产物。当前仍没有 Tauri/ReaderBridge 接口、启动消费、结果日志或执行按钮，真实用户数据
不会自动替换。
...
- 2026-08-24：新增 core-only 一次性启动恢复调度，明确确认后持锁刷新外部回滚点并重新预检，再原子发布固定
  JSON 请求；拒绝重复、来源漂移与不完整确认，发布前失败精确清理。总计 11 个隔离事务/调度测试全绿。
- 2026-08-24：新增隔离启动期恢复事务内核与 6 个故障注入测试，覆盖预变更残留、阶段中断、复核失败、
  回滚重试、外部备份消失后的本地回滚及来源已激活后的幂等收尾；未接 Tauri/UI，也未触碰真实 app data。
- 2026-08-24：恢复准备新增只读预检：限制外部普通凭据，复核路径/状态、两份完整备份与 manifest 摘要，
  并检查 staging 加安全余量的目标卷空间；结果固定未授权/未执行，真正执行前仍必须刷新当前数据回滚点。
- 2026-08-24：恢复前置链可创建并复核当前数据的外部回滚点，写入独立 JSON 准备凭据并钉住来源/回滚
  manifest 摘要；按钮和 DTO 明确“不恢复”，失败清理仅限本次新建回滚目录，活动数据保持不变。
- 2026-08-23：备份恢复前置链新增只读事务计划：当前与备份的五类内容计数并排显示，估算回滚点，固定要求
  外部预备份和重启，较新版本备份阻断；活动数据库和文件系统均不被修改。
- 2026-08-23：备份链补齐只读校验和内容预览；core 校验 schema、路径、精确文件集合、大小/SHA-256、
  两份 SQLite `quick_check` 与来源版本，桌面书架新增“校验备份”和只读详情弹窗，不提供恢复操作。
- 2026-08-22：桌面书库新增“备份数据”，输出 schema v1 独立目录与 SHA-256 清单；reading-core 单测覆盖
  一致性快照、排除、目标越界和失败清理，真实 P0 在隔离数据目录生成 8 个载荷文件并完成重启恢复复验。
- 2026-08-22：书架新增筛选、排序与结果计数；插件在线结果期间整理控件会禁用并显示在线摘要。隔离 P0
  夹具已验证 Vol.1 最近阅读排首位、本地未读只保留 Vol.2、远程筛选显示 0/2 和书名顺序，真实 WebView2
  smoke 同时收紧为等待书架渲染完成后再验收。
- 2026-08-22：经维护者明确同意，v0.7.3 已正式公开为非预发布的 Latest Release；tag 精确指向构建提交
  `60c0868fa4d60c131ad431798dc9ac7981308816`。公开五资产重新下载后统一验收通过，Latest 别名清单
  SHA-256 一致。正式 v0.7.2 经应用内详情弹窗完成安全取消、再次确认、下载、验签、quiet 安装、自动重启和
  v0.7.3 同版本检查，没有出现语言选择框；两份数据库长度与 SHA-256 全程不变。
- 2026-08-22：v0.7.3 修复通过 PR #61 合并到 `main@60c0868`，PR/main 两套 Windows CI 全绿；从该固定
  提交生成并签署严格五资产，统一验收、静默安装、安装版真实窗口、首次缓存迁移、静默卸载、数据保留和
  重装均通过。GitHub Draft Release 固定到构建提交，远端五资产摘要匹配，公开 tag 不存在；公开 Latest 仍为 v0.7.2。
- 2026-08-22：从正式安装的 v0.7.1 经公开清单完成到 v0.7.2 的检查、发布说明、安全取消、再次确认、
  下载、签名接受、安装、自动重启和新 UI 复验；随后同版本检查返回“当前已是最新版本”。`reader.db` 与
  `library.sqlite` 的长度和 SHA-256 全程不变。排查并修复桌面旧 PWA 缓存污染及多语言 NSIS `passive`
  语言提示阻塞；Windows 首次启动迁移只删除 WebView2 Cache/Code Cache/Service Worker，不触碰站点存储或书库。
  开发版本升至 v0.7.3；公开 v0.7.2 五资产未修改。
- 2026-08-21：v0.7.2 已从 `main` 固定提交 `9c7d36f` 完成正式签名构建和五资产统一验收；安装版真实窗口
  更新检查、静默装卸及两份数据库哈希保留通过。Draft Release 固定到该提交，远端五资产摘要匹配且公开 tag
  不存在；随后经维护者明确同意已正式公开为 Latest Release，tag 固定到该构建提交，五资产摘要未变化。
- 2026-08-22：v0.7.2 已正式公开为非预发布的 Latest Release；远端 tag 精确指向
  `9c7d36f8f57694ebceaf02915ce1f3462ad171b5`，五个公开资产的名称、数量与 GitHub SHA-256 digest 均和
  正式候选一致。下一步从正式安装的 v0.7.1 完成检查、详情、下载、验签、安装、重启与数据保留闭环。
...
- 2026-08-21：v0.7.1 已从 `main` 合并提交 `1929999` 完成正式签名构建，五资产通过统一公钥侧验收；
  静默安装、安装版真实窗口与更新入口、静默卸载均通过，两份既有数据库长度与 SHA-256 前后不变。
  Draft Release 当时已固定到该完整提交，远端五资产摘要逐项一致且公开 tag 不存在；随后经维护者确认公开。
