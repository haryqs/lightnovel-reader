...
1. 产品定位已升级为本地优先轻小说平台：阅读器是核心模块，但不是完整边界。
2. 当前主线是 v0.6/v0.6.5：OPDS、结构化错误码、合法开放资源获取、阅读方式选择。
3. 桥接协议已进入 `1.0-rc.1` 冻结候选：后续默认只新增消息/可选字段，不随意改名、删字段或改变既有语义。
4. 下一步优先完成协议冻结候选 review、目标机器便携包抽检、NSIS 卸载保留数据验证；功能线再进入 v0.7 插件运行时 / ToS 门控。
5. 合法边界不变：公共版权、开放授权、用户自有资源可站内阅读；商业/受保护/未知授权正文只保留 metadata 与官方入口。
