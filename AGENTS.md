...
2. **当前主线 v0.7 插件运行时**（进行中 60%）：manifest 策略/zip 安装/官方仓库索引/下载校验链路已完成；待补真实窗口 smoke + host API QuickJS 集成 + UI 区分。
3. 桥接协议已进入 `1.0-rc.1` 冻结候选：后续默认只新增消息/可选字段，不随意改名、删字段或改变既有语义。
4. 合法边界不变：公共版权、开放授权、用户自有资源可站内阅读；商业/受保护/未知授权正文只保留 metadata 与官方入口。
5. **注意**：当前不执行插件 JS，不引入 QuickJS。host API 有纯 Rust DTO 骨架但不执行。
