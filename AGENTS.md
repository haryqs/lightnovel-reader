...
1. 产品定位：本地优先轻小说平台。阅读器是核心模块，但不是完整边界。
2. **当前主线 v0.7 首发维护**：v0.7.1 已正式公开，v0.7.2 签名五资产与 Draft Release 已就绪但尚未公开；QuickJS、正式 `source.*`、合法开放 EPUB 获取、真实离线窗口 smoke、每域限速/ToS、Ed25519 包验签、Gutenberg 正式仓库及应用内 updater/安装器公开分发均已落地；下一步经维护者单独明确同意后公开 v0.7.2，再以公开 v0.7.1 为旧版本完成完整用户界面更新闭环。
3. **v1.0 四阶段架构已全部落地**：WASM网页端 / 自托管同步服务 / 桌面独立化 / GPU翻页+PWA。
4. 桥接协议已进入 `1.0-rc.1` 冻结候选：后续默认只新增消息/可选字段。
5. 合法边界不变：公共版权、开放授权、用户自有资源可站内阅读。
