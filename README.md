...
当前主线在 v0.6：OPDS、结构化错误码和平台化书库能力收口中。`opds.*` 与 `library.*` 命令已返回结构化 `BridgeError { code, message, details? }`，前端通过 `formatError` 统一展示。下一步重点是阅读方式选择、合法开放资源 acquire 体验、外部阅读器打开能力，以及继续迁移 `book.*`、`annotation.*`、`reading.*`。
